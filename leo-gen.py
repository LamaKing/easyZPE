...
if __name__ == "__main__":
    ...
    calculator_input = del_magnetisations(calculator_input) # delete magnetisations from input. ASE will overrite.
    ...
    for ia, atm in enumerate(geom):
        ...
        for j in range(3): # x y z
            ...
            # --- positive displ
            c_geom = geom.copy()
            if debug: print('before %.3f %.3f %.3f' % tuple(c_geom[ia].position))
            c_geom[ia].position[j] += displ
            if debug: print('after+ %.3f %.3f %.3f' % tuple(c_geom[ia].position))
            # --- --- write
